...
conn = psycopg2.connect(dbname="dq", user="hud_admin", password="abc123")
cur = conn.cursor()
query = "EXPLAIN SELECT COUNT(*) FROM homeless_by_coc WHERE year > '2012-01-01'"
cur.execute(query)
pp.pprint(cur.fetchall())
...
conn = psycopg2.connect(dbname="dq", user="hud_admin", password="abc123")
cur = conn.cursor()
query = "EXPLAIN (format json, ANALYZE) SELECT COUNT(*) FROM homeless_by_coc WHERE year > '2012-01-01'"
cur.execute(query)
pp.pprint(cur.fetchall())
...
conn = psycopg2.connect(dbname="dq", user="hud_admin", password="abc123")
cur = conn.cursor()
query = "EXPLAIN (ANALYZE, format json) DELETE FROM state_household_incomes"
cur.execute(query)
conn.rollback()
pp.pprint(cur.fetchall())
